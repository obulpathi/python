...
for f in $(grep -lr project $PROJECT_PATH); do
    sed -e "s/project/${PROJECT_NAME}/g" < $f > $f.tmp && mv $f.tmp $f
done

# replace all references to Project in project files
for f in $(grep -lr Project $PROJECT_PATH); do
    sed -e "s/Project/${PROJECT_NAME_PROPER}/g" < $f > $f.tmp && mv $f.tmp $f
done

# rename project module directory
mv ${PROJECT_PATH}/project ${PROJECT_PATH}/${PROJECT_NAME}

# echo blurb
echo "projecting of $PROJECT_NAME to $PROJECT_PATH was successful."
